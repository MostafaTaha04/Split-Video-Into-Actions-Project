...
> **Use `detector_kit.py` for steps 2–4 below — it replaces the manual work.**
>
> ```bash
> python extract_training_frames.py --videos *.mp4 --output dataset/images --every 0.5
> python detector_kit.py classes --output dataset/classes.txt   # class list for your labeller
> #   ... draw the boxes ...
> python detector_kit.py prepare  --root dataset                # split + data.yaml
> python detector_kit.py validate --root dataset                # catch broken labels
...
> ```
>
> Two things the kit does that hand-rolling gets wrong:
> **it splits by source video, not at random** (frames 0.5 s apart are near-duplicates,
> so a random split leaks them into validation and reports an accuracy you will not
> reproduce), and **`compare` measures the result against the zero-shot baseline** —
> without that, "we trained a detector" is an assertion, not a finding.
