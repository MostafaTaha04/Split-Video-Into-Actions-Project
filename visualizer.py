...
class Visualizer:
    ...
    def draw_frame(self, frame: np.ndarray,
                   ...
                   draw_flow: bool = False) -> np.ndarray:
    ...
    def _draw_flow_overlay(self, frame: np.ndarray,
                           flow_data: FlowData) -> np.ndarray:
    ...
    def _draw_segment_info(self, frame: np.ndarray, segment: ActionSegment):
        ...
        cv2.putText(frame, text, (20, h - 65),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

        if segment.tools_used:
            tools_text = f"Tools: {', '.join(segment.tools_used)}"
            cv2.putText(frame, tools_text, (20, h - 40),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1)

        conf_text = f"Confidence: {segment.confidence:.2f} | Motion: {segment.avg_motion_energy:.1f}"
        cv2.putText(frame, conf_text, (20, h - 18),
    ...
    def generate_timeline(self, segments: List[ActionSegment],
        ...
        for i, segment in enumerate(segments):
            ...
            cv2.putText(timeline, label, (mid_x - 20, 75),
        ...
        for boundary in boundaries:
            ...
            cv2.putText(timeline, f"{boundary.confidence:.2f}",
                       (x - 10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1)

        # Draw feature graph (transition scores)
        if features:
            graph_top = 130
            graph_bottom = 270
            graph_height = graph_bottom - graph_top

            cv2.rectangle(timeline, (0, graph_top - 10), (width, graph_bottom + 10),
                         (20, 20, 20), -1)
            cv2.putText(timeline, "Transition Score", (10, graph_top + 15),
            ...
            if scores:
                ...
                # Draw threshold line
                thresh_y = graph_bottom - int((self.threshold_val(boundaries) / max_score) * graph_height)
        ...
        for i in range(0, int(total_duration) + 1, max(1, int(total_duration / 10))):
            ...
            cv2.putText(timeline, f"{i}s", (x, 118),
    ...
    def export_feature_csv(self, features: List[FrameFeatures],
        ...
        with open(path, 'w') as f:
            ...
            for feat in features:
                row = [str(feat.frame_idx), f"{feat.timestamp:.3f}"]
                row.extend([
