...
class ObjectDetector:
    ...
    def detect(self, frame: np.ndarray) -> List[DetectedObject]:
        ...
        if self.detector_mode == "none":
            ...
            # Custom detections PLUS workspace ROIs, so segmentation keeps the
            # ROI fallback even when the trained model misses a frame.
            detections = (
    ...
    def _detect_open_vocab(self, frame: np.ndarray) -> List[DetectedObject]:
        ...
        try:
            # In debug mode, run at a very low floor so we can see EVERY
            # candidate the model proposes (incl. sub-threshold), then still
            # only keep boxes >= self.confidence for the real result.
            floor = min(self.confidence, 0.01) if self.debug else self.confidence
    ...
    def _debug_print_raw(self, results):
        """Print every raw YOLO-World candidate (label + confidence) for one frame."""
        rows = []
        for result in results:
            boxes = result.boxes
            if boxes is None:
                continue
            names = getattr(result, "names", None) or self.open_vocab_names
            for box in boxes:
                cls_id = int(box.cls[0])
                name = names.get(cls_id, str(cls_id)) if isinstance(names, dict) else str(cls_id)
                rows.append((name, float(box.conf[0])))
        rows.sort(key=lambda r: r[1], reverse=True)
        if rows:
            shown = ", ".join(f"{n}:{c:.3f}" for n, c in rows[:12])
            print(f"[debug] frame {self.call_count}: {shown}")
        else:
            print(f"[debug] frame {self.call_count}: NO candidates at all (model sees nothing)")
    ...
    def _detect_workspace_regions(self, frame: np.ndarray) -> List[DetectedObject]:
        ...
        detections.append(self._make_detection("motherboard_workspace", workspace_box, 0.78, source="workspace"))

        socket_box = self._estimate_cpu_socket_box(frame, workspace_box)
        detections.append(self._make_detection("cpu_socket_region", socket_box, 0.55, source="workspace"))

        motion_box = self._estimate_active_motion_box(frame)
        if motion_box is not None:
            detections.append(self._make_detection("active_motion_region", motion_box, 0.65, source="workspace"))
    ...
    def _estimate_cpu_socket_box(self, frame, workspace_box):
    ...
    def _estimate_active_motion_box(self, frame):
        h, w = frame.shape[:2]
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        gray = cv2.GaussianBlur(gray, (5, 5), 0)
    ...
    def _deduplicate_detections(self, detections, iou_threshold=0.75):
        ...
        if not detections:
            return []
        source_priority = {"open_vocab": 3, "yolo": 3, "workspace": 1, "unknown": 0}
        detections = sorted(detections, key=lambda d: (source_priority.get(d.source, 0), d.confidence), reverse=True)
        kept = []
    ...
    @staticmethod
    def _expand_box(box, shape, pad_ratio=0.05):
        h, w = shape[:2]
        x1, y1, x2, y2 = box
        bw = x2 - x1; bh = y2 - y1
    ...
    @staticmethod
    def _make_detection(class_name, box, confidence, source="workspace"):
        x1, y1, x2, y2 = box
        center = np.array([(x1 + x2) / 2, (y1 + y2) / 2], dtype=float)
        area = float(max(0, x2 - x1) * max(0, y2 - y1))
        return DetectedObject(class_name=class_name, confidence=confidence,
                              bbox=(int(x1), int(y1), int(x2), int(y2)),
                              center=center, area=area, source=source)

    def _update_history(self, detections):
    ...
    def filter_tools(self, detections):
    ...
    def get_new_objects(self, detections, iou_threshold=0.5):
    ...
    def get_disappeared_objects(self, detections, iou_threshold=0.5):
        disappeared = []
        for prev in self.previous_detections:
            still_present = False
            for det in detections:
