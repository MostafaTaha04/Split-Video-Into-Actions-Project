...
class ActionSplitterPipeline:
    ...
    def run(self) -> dict:
        ...
        print("Modules: Hand Tracking + Workspace/Object Detection + Interaction Tracking")
    ...
    def _process_frames(self) -> list:
        ...
        frames_data = []
        total = max(1, self.video_loader.total_frames // self.video_loader.frame_skip)

        for frame_idx, frame in tqdm(self.video_loader.frames(), total=total, desc="Processing"):
            ...
            curvature = 0.0
            for hand in hands:
                curvature = max(curvature, self.hand_tracker.get_trajectory_curvature(hand.handedness))
    ...
    def _generate_outputs(self, frames_data: list, segments: list,
        ...
        if self.config.export_clips:
            ...
            for segment in segments:
                ...
                self.video_loader.export_clip(segment.start_frame, segment.end_frame, clip_path)
    ...
    def _run_evaluation(self, segments, boundaries) -> dict:
        ...
        print("\n--- EVALUATION ---")
        results = self.evaluator.evaluate(segments, boundaries, tolerance=self.config.boundary_tolerance)
...
def main():
    parser = argparse.ArgumentParser(description="Split egocentric video into action steps")
    parser.add_argument("--video", "-v", required=True, help="Input video path")
    parser.add_argument("--output", "-o", default="output/", help="Output directory")
    parser.add_argument("--threshold", "-t", type=float, default=0.32,
    ...
    parser.add_argument("--detector", choices=["workspace", "yolo", "hybrid", "none"],
                        default="workspace",
                        help="workspace is recommended for CPU/motherboard videos without a custom model")
    parser.add_argument("--model", type=str, default=None,
                        help="Path to custom YOLO model weights, e.g. hardware_model.pt")
